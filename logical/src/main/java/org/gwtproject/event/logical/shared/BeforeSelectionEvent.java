 ...
 */
public class BeforeSelectionEvent<T> extends Event<BeforeSelectionHandler<T>> {

  /** Handler type. */
  private static Type<BeforeSelectionHandler<?>> TYPE;

  /**
   * Fires a before selection event on all registered handlers in the handler manager. If no such
  ...
  public static <T> BeforeSelectionEvent<T> fire(HasBeforeSelectionHandlers<T> source, T item) {
    ...
    if (TYPE != null) {
      BeforeSelectionEvent<T> event = new BeforeSelectionEvent<>();
      event.setItem(item);
